...
<tr><td class="lefttd">Age (Min-18 and Max-59)</td><td><input type="number" name="t2" required="required" pattern="[0-9]{18,59}" min="18" max="59" title="Please enter only numbers between 18 to 59 for age" /></td></tr>
...
<tr><td class="lefttd">Upload Picture</td><td><input type="file" name="t8" /></td></tr>

<tr><td class="lefttd">Upload Medical Certificate </td><td><input type="file" name="t9" required="required" /></td></tr>
...
{
	{
		$target_dir1 = "Admin/doner_pic/";
		$target_file1 = $target_dir1 . basename($_FILES["t8"]["name"]);
		$uploadOk = 1;
	...
	{
		$target_dir2 = "Admin/donor_medical/";
		$target_file2 = $target_dir2 . basename($_FILES["t9"]["name"]);
		$uploadOk = 1;
		$certificateFileType = pathinfo($target_file2,PATHINFO_EXTENSION);
		// Check if certificate file is a actual file or fake 

  		$check = getimagesize($_FILES["t9"]["tmp_name"]);
  		if($check !== false) 
		{
   			//  echo "File is an Original - " . $check["mime"] . ".";
        		$uploadOk = 1;
   		} 
		else 
		{
		...
		{
		if(move_uploaded_file($_FILES["t8"]["tmp_name"] , $target_file1))
		{			
			if(move_uploaded_file($_FILES["t9"]["tmp_name"] , $target_file2))
			{
				$cn=makeconnection();
				$s="insert into donarregistration(name,gender,age,mobile,b_id,email,pwd,pic,medical_certificate) values('" . $_POST["t1"] ."','" . $_POST["r1"] . "','" . $_POST["t2"] . "','" . $_POST["t3"] . "','" . $_POST["t4"] . "','" . $_POST["t5"] . "','" . $_POST["t7"] .  "','" . basename($_FILES["t8"]["name"])  ."','" . basename($_FILES["t9"]["name"])  ."')";
		
				//$s="INSERT INTO donarregistration(name,gender,age,mobile,b_id,email,pswd,pic,medical_certificate) VALUES ([value-1],[value-2],[value-3],[value-4],[value-5],[value-6],[value-7],[value-8],[value-9],[value-10])"
				mysqli_query($cn,$s);
				mysqli_close($cn);
				if($s>0)
				{
