...
$sql_select  = "SELECT email  FROM contacts WHERE row_id = $row_id";
...
if (mysqli_num_rows($result) > 0) {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    $email = $row["email"];
     ...
     $mail->Subject = 'Regarding Enquiry';
     $mail->Body    = '<h1>We have gone through your enquiry and will sort it out soon. Thank you!</h1>';
...
$sql_delete = "DELETE FROM contacts WHERE row_id = $row_id";
if (mysqli_query($conn, $sql_delete)) {
  echo  "<script>alert('Enquiry Notified');</script>";
